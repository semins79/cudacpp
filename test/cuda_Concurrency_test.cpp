...
int main(int argc, char ** argv)
{
  ...
  cudacpp::Stream stream2;
  cudacpp::Event evt0, evt1, evt2, evt3;
  ...
  evt2.record(&stream2);
  cudacpp::Runtime::memcpyDtoHAsync(cpuMem0, gpuMem0, NUM_FLOATS * sizeof(float), &stream2);
  cudacpp::Runtime::memcpyDtoHAsync(cpuMem1, gpuMem1, NUM_FLOATS * sizeof(float), &stream2);
  evt3.record(&stream2);
  evt3.sync();
  ...
  printf("memcpy + kernel + memcpy  took %.4f s\n", evt0.getElapsedSeconds(&evt1));
  printf("concurrent memcpy         took %.4f s\n", evt2.getElapsedSeconds(&evt3));
